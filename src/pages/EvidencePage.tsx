...
function QrCode({ evidenceId, size = 120 }: { evidenceId: string; size?: number }) {
  ...
  useEffect(() => {
    ...
    fetch(getEvidenceQrUrl(evidenceId), {
      headers: { Authorization: `Bearer ${localStorage.getItem('wafpass_access_token') ?? ''}` },
      signal: ctrl.signal,
    })
      .then(r => r.ok ? r.text() : null)
      .then(text => { if (text && !ctrl.signal.aborted) setSvg(text) })
      .catch(() => {})
    return () => ctrl.abort()
  }, [evidenceId])
      ...
      <div style={{ padding: '1rem 1.25rem', display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.75rem', flexWrap: 'wrap' }}>
            ...
            <a
              href={getEvidenceReportUrl(ev.id)}
